...
public class MypageDAO {
	...
	public void orderUpdate(String address, int count, String name, int beCnt, String pid) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		try
		{
			conn = JdbcDAO.con();
			pstmt = conn.prepareStatement("update purchase set address=?, count=?, odate=current_date where id=?;");
			...
			if(count > beCnt) {
				int num = count - beCnt;
				pstmt = conn.prepareStatement("update product set max=max-? where pid=?;");
				pstmt.setInt(1, num);
