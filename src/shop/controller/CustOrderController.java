...
public class CustOrderController implements Controller {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(flower == null) {
			request.setAttribute("error", "꽃을 선택해주세요.");
			HttpUtil.forward(request, response, "/customOrder.jsp");
			return;
		}
		if(type.equals("cus1")) {
			if(flower.length > 1) {
				request.setAttribute("error", "type1 : 하나만 선택해주세요.");
				HttpUtil.forward(request, response, "/customOrder.jsp");
				return;
			}
		}else if( type.equals("cus2")) {
			if(flower.length > 3) {
				request.setAttribute("error", "type2 : 2~3개만 선택해주세요.");
				HttpUtil.forward(request, response, "/customOrder.jsp");
				return;
			}else if(flower.length < 2) {
				request.setAttribute("error", "type2 : 2개 이상 선택해주세요.");
				HttpUtil.forward(request, response, "/customOrder.jsp");
				return;
			}
		}
